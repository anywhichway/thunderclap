...
name form is recommended so that the name parallels the names of scripts generated by the Thunderclap script `thunderclap`.
...
`webpack.config.js` assumes the file in up one level in the directory tree. You may need to edit `webpack.config.js`
